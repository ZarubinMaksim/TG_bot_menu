function Card ({ item, addToOrderList, removeFromOrderList, amount }) {
  ...
  return (
    <div className="border border-green-200 hover:bg-green-100 transition cursor-pointer rounded-lg flex flex-col justify-between items-center w-2/5 p-3 items-center">
      <img className="rounded overflow-hidden object-cover w-40 h-40" src={require(`../images/${item.img}`)}></img>
      <p className="text-md text-center mt-3">{item.name}</p>
